...
function New-KeyFile {
    ...
    Downloaded from: https://www.sqlshack.com/how-to-secure-your-passwords-with-powershell/
...
function Get-PSCredential {
    <# 
    .SYNOPSIS 
    Creates a PSCredential object using the encrypted password file.
    
    .DESCRIPTION 
    Pulls the encrypted password utilizing the key file provided.
    ...
    .EXAMPLE   
    Get-PSCredential -PwdFile C:\MyPassword.txt -KeyFile C:\MyKey. -User sa
    
    Description
    Retrieves the password found in C:\MyPassword.txt and decrypts it using the key found in C:\MyKey.key. 
    Outputs the SecureString object to be used to create the PSCredential object.
    #>
    [cmdletbinding()]
    param(
        [string]$PwdFile,
    ...
    $keyValue = Get-Content $KeyFile
    $pwd = Get-Content $PwdFile | ConvertTo-SecureString -Key $keyValue
    $PSCred = New-Object System.Management.Automation.PSCredential($User,$pwd)
    return $PSCred
}
function Reset-Container {
   ...
   .SYNOPSIS 
    ...
    .NOTES 
    Author: Adam Anderson @sqlpioneer

    .EXAMPLE   
    Reset-Container containername
